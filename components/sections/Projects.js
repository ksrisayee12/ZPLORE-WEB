...
const PROJECTS = [
  {
    ...
    name: "DEVSEC",
    tag: "Security · AI",
    desc: "AI-augmented secure development platform. Real-time code intelligence, threat modeling and policy enforcement woven into the IDE.",
    size: "lg",
  },
  {
    id: "zip",
    name: "ZIP",
    tag: "Compression · R&D",
    desc: "Next-gen neural compression pipeline. 40% smaller payloads, lossless, hardware-accelerated.",
    size: "md",
  },
  {
    id: "jarvis",
    name: "JARVIS",
    tag: "Agentic · OS",
    desc: "A personal computing layer that operates your tools. Memory, reasoning, and action — unified.",
    size: "wide",
  },
  {
    id: "rag",
    name: "College RAG Intelligence",
    tag: "Retrieval · Education",
    desc: "A retrieval system purpose-built for academia. Multi-modal indexing across lectures, papers and labs.",
    size: "md",
  },
  {
    id: "sadt",
    name: "Secure Audio Data Transfer",
    tag: "Crypto · Acoustic",
    desc: "Encrypted, air-gapped data transfer via acoustic modulation. Verified at 28 kbps over ultrasonic carriers.",
    size: "md",
  },
];
...
function ProjectCard({ project, index }) {
  ...
  const colSpan =
    project.size === "lg"
      ? "col-span-12 md:col-span-7 row-span-2 min-h-[420px] md:min-h-[560px]"
      : project.size === "wide"
        ? "col-span-12 min-h-[360px]"
        : "col-span-12 md:col-span-" +
          (project.size === "md" ? "5" : "6") +
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage:
            "url(\"data:image/svg+xml;utf8,<svg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'><filter id='n'><feTurbulence type='fractalNoise' baseFrequency='0.95' numOctaves='2'/></filter><rect width='100%25' height='100%25' filter='url(%23n)'/></svg>\")",
        }}
      />
        ...
        <svg viewBox="0 0 600 400" className="absolute inset-0 w-full h-full">
          <g stroke="white" strokeWidth="1" fill="none" opacity="0.6">
            {Array.from({ length: 24 }).map((_, i) => {
              ...
              const seed = i * 0.5;
              const y = roundSvgValue(100 + Math.sin(seed) * 40);
              const h = roundSvgValue(200 - Math.sin(seed) * 60);
              return (
                <rect key={i} x={40 + i * 22} y={y} width="10" height={h} />
              );
            })}
          </g>
        </svg>
        ...
        <svg viewBox="0 0 1000 400" className="absolute inset-0 w-full h-full">
          <g stroke="white" strokeWidth="1" fill="none" opacity="0.6">
            {Array.from({ length: 14 }).map((_, i) => (
              <circle key={i} cx={100 + i * 60} cy="200" r={20 + i * 4} />
            ))}
            ...
            <path
              d={[
                ...
                ...Array.from({ length: 60 }).map(
                  (_, i) =>
                    `L ${i * 10} ${roundSvgValue(200 + Math.sin(i * 0.6) * Math.cos(i * 0.2) * 70)}`,
                ),
              ].join(" ")}
            />
            <path
              d={[
                ...
                ...Array.from({ length: 60 }).map(
                  (_, i) =>
                    `L ${i * 10} ${roundSvgValue(220 + Math.sin(i * 0.4) * 50)}`,
                ),
              ].join(" ")}
              opacity="0.4"
            />
